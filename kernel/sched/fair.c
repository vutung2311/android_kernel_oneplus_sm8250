 ...
 */
#define fits_capacity(cap, max, mar) ((cap) * (mar) < (max) * 1024)
...
{
	return !fits_capacity(cpu_util(cpu), capacity_orig_of(cpu),
		    ...
		    int sibling_count_hint)
{
	...
	if (sd_flag & SD_BALANCE_WAKE) {
		...
		want_affine = !wake_wide(p, sibling_count_hint) && cpumask_test_cpu(cpu, &p->cpus_allowed);
	}
