...
while [[ $# -gt 1 && $1 != FORCE ]]; do
	...
	for overlay in "$TMPDIR"/overlay-$total.dtbo*; do
		...
		sed -i -e "0,\%^\(\s*\)model\s*=.*%{s%%\1model = $model%}" "$TMPDIR/combined-$total.dts"
		sed -i -e "0,\%^\(\s*\)compatible\s*=.*%{s%%\1compatible = $compatible%}" "$TMPDIR/combined-$total.dts"
		sed -i -e "0,\%^\(\s*\)qcom,board-id\s*=.*%{s%%\1qcom,board-id = $board_id%}" "$TMPDIR/combined-$total.dts"
